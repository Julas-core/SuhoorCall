...
class NearbyConnectionsTransport implements P2pTransport {
  ...
  Future<void> startDiscovery({String? targetPeerId}) async {
    ...
    _discoveryTimer = Timer(const Duration(milliseconds: 700), () {
      if (!_isDiscovering || _eventsController.isClosed) {
        return;
      }

      final discoveredPeerId = _targetPeerId ?? 'nearby-peer-1';
      _eventsController.add(
        P2pTransportEvent(
          type: P2pTransportEventType.peerDiscovered,
          peer: P2pPeer(id: discoveredPeerId, displayName: 'Nearby Brother'),
        ),
      );
    });
  }
